...
const requiredFields = ['id', 'title', 'starttime', 'endtime'];
...
function syncFromCalendar() {
  ...
  const eventFound = new Array(data.length);
  ...
  const sheetEventIds = data.slice(1).map(function(row) {
    return row[idIdx];
  });

  // Loop through calendar events
  for (let cidx = 0; cidx < calEvents.length; cidx++) {
    const calEvent = calEvents[cidx];
